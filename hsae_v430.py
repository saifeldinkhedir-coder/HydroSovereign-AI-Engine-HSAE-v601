...
<div class='mono'>
  ...
  <tr><td>🌍 Countries</td><td>{', '.join(basin.get('country',['—'])[:4])}</td></tr>
    ...
    if _FOLIUM:
        ...
        with c_map:
            import plotly.express as _px, pandas as _pd
            _df = _pd.DataFrame([{"lat":v["lat"],"lon":v["lon"],"name":k,
                                   "size":18 if k==basin_name else 7}
                                  for k,v in GLOBAL_BASINS.items()])
            _fig = _px.scatter_mapbox(_df,lat="lat",lon="lon",hover_name="name",
                                      size="size",zoom=2,height=360,
    ...
    # Guard: if basin is None or not a dict, fall back to first basin
    if not isinstance(basin, dict):
        basin = GLOBAL_BASINS[ALL_NAMES[0]]
    ...
    with tab_mb:
        ...
        _ba = basin.get('bathy_a', 0.038) or 0.038
        _bb = basin.get('bathy_b', 1.12)  or 1.12
        st.latex(
            rf"V = {_ba:.3f}\times A^{{{_bb:.2f}}}"
