...
class HSAEDashboardPanel(QDockWidget):
    ...
    def _compute(self, b):
        ...
        if hifd>=25: arts.append('Art.20 EnvFlow')
    ...
    def _build_ui(self):
        ...
        # Header
        ml.addWidget(self._lbl("🌊 HydroSovereign AI Engine v6.01",
            f"color:{BLUE};font-weight:bold;font-size:13px"))
        ml.addWidget(self._lbl("Seifeldin M.G. Alkedir · ORCID: 0000-0003-0821-2991",
        ...
        for i,(key,col) in enumerate(defs):
            ...
            fl  = QVBoxLayout(fr); fl.setContentsMargins(4,3,4,3)
            fl.addWidget(self._lbl(key, f"color:{col};font-size:9px;font-weight:bold"))
            v = self._lbl("—", f"color:{WHT};font-size:13px;font-weight:bold")
            fl.addWidget(v)
            g.addWidget(fr,r,c)
            self.vals[key] = v

        pb_f  = QFrame(); pb_l = QVBoxLayout(pb_f); pb_l.setContentsMargins(0,4,0,0)
        self.bar_atdi = self._bar(ORG)
        self.bar_hifd = self._bar("#e3b341")
        for lab,bar in [("ATDI",self.bar_atdi),("HIFD",self.bar_hifd)]:
            rl = QHBoxLayout()
            rl.addWidget(self._lbl(f"{lab}:", f"color:{GRAY};font-size:9px"), 0)
            rl.addWidget(bar, 1)
            pb_l.addLayout(rl)
        ...
        # Tab 2: Physical
        t2 = QWidget()
        g2 = QGridLayout(t2); g2.setSpacing(4)
        self.phys = {}
        ...
        for i,(key,ico) in enumerate(pdefs):
            ...
            fr  = self._frame()
            fl  = QVBoxLayout(fr); fl.setContentsMargins(4,3,4,3)
            fl.addWidget(self._lbl(f"{ico} {key}", f"color:{GRAY};font-size:9px"))
            v = self._lbl("—", f"color:{WHT};font-size:12px;font-weight:bold")
            fl.addWidget(v); g2.addWidget(fr,r,c); self.phys[key] = v
    ...
    def _bar(self, color):
        b = QProgressBar()
        b.setRange(0,100); b.setFixedHeight(7); b.setTextVisible(False)
        b.setStyleSheet(
    ...
    def _report(self):
        b = self.current; d = self._compute(b)
        path, _ = QFileDialog.getSaveFileName(
            None, "Export Report",
            f"HSAE_{b.get('name','').replace(' ','_').replace('/','_')}", "Text (*.txt)")
        if not path: return
        ...
        with open(path,'w',encoding='utf-8') as f:
            ...
            f.write(f"Author: Seifeldin M.G. Alkedir | ORCID: 0000-0003-0821-2991\n\n")
