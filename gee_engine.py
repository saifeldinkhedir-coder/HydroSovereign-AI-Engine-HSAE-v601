...
def _ensure_ee(project: Optional[str] = None) -> bool:
    ...
    Priority:
      1. Streamlit Secrets (Service Account) — Streamlit Cloud deployment
      2. Local credentials (earthengine authenticate) — local development
    ...
    # ── Method 1: Streamlit Secrets (Service Account) ────────────────────────
    try:
        import streamlit as st
        import json
        if "gee" in st.secrets:
            ...
            key_dict = {
                ...
                "client_email":                gee_cfg.get("client_email", ""),
                "client_id":                   gee_cfg.get("client_id", ""),
                "auth_uri":                    gee_cfg.get("auth_uri", "https://accounts.google.com/o/oauth2/auth"),
                "token_uri":                   gee_cfg.get("token_uri", "https://oauth2.googleapis.com/token"),
                "auth_provider_x509_cert_url": gee_cfg.get("auth_provider_x509_cert_url", ""),
                "client_x509_cert_url":        gee_cfg.get("client_x509_cert_url", ""),
                "universe_domain":             gee_cfg.get("universe_domain", "googleapis.com"),
            }
        ...
        warnings.warn(f"GEE Secrets init failed: {exc}", RuntimeWarning)

    # ── Method 2: Local credentials ──────────────────────────────────────────
    try:
        if project:
