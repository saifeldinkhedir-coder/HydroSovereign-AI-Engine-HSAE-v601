...
import pytest
from hydrosovereign.provenance import DataPoint, DataQuality, INSUFFICIENT_DATA
from hydrosovereign.ingestion import DataRegistry
...
class TestAWGIComposite:
    def test_weights_sum_to_one(self):
        from hydrosovereign.indices import AWGI_DEFAULT_WEIGHTS
        assert abs(sum(AWGI_DEFAULT_WEIGHTS.values()) - 1.0) < 1e-9

    def test_factors_normalised_0_1(self):
        from hydrosovereign.indices import compute_awgi
    ...
    def test_sensitivity_runs(self):
        from hydrosovereign.indices import awgi_sensitivity
