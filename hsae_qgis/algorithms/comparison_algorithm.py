"""
comparison_algorithm.py — HSAE v6.0.10 QGIS Processing Algorithm
...
class MultiBasinComparisonAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        names = [b.strip() for b in self.parameterAsString(
            parameters, self.BASINS, context).split(',')]
        outcsv = self.parameterAsFileOutput(parameters, self.OUT_CSV, context)
        outhtml = self.parameterAsFileOutput(
            parameters, self.OUT_HTML, context)
        ...
        with open(outcsv, 'w', encoding='utf-8') as f:
            f.write(
                'Basin,Countries,ATDI%,HIFD%,NSE,KGE,CI,P_Negotiation,Dispute,Articles\n')
            for r in results:
                f.write(
                    f"{
                        r['name']},{
...
<title>HSAE v6.0.10 — Multi-Basin Comparison</title>
<style>
...
<h1>🌊 HSAE v6.0.10 — Multi-Basin Comparison Report</h1>
...
HSAE v6.0.10 · {len(results)} basins · UNWC 1997 · TFDD/ICOW</p>
</body></html>""")
