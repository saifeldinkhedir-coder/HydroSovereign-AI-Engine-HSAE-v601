...
def compute_atdi(runoff_c: float, cap_bcm: float,
    ...
    return round(min(95.0, max(5.0,
                               10.0
                               + min(float(cap_bcm)/8.5, 11.0)
                               + float(dispute_level)*4.8
                               + (float(n_countries)-2)*2.0
                               + (1.0-float(runoff_c))*6.0
                               )), 1)
...
def compute_ahifd(runoff_c: float, cap_bcm: float,
    ...
    return round(min(80.0, max(3.0,
                               3.0
                               + min(float(cap_bcm)/18.0, 6.0)
                               + (1.0-float(runoff_c))*5.0
                               + float(dispute_level)*2.0
                               + (float(n_countries)-2)*1.5
                               )), 1)
...
def compute_conflict_index(atdi: float, ahifd: float,
    ...
    return round(min(1.0, max(0.0,
                              0.40*atdi/100 + 0.25*dispute_level/4.0
                              + 0.20*ahifd/80.0 + 0.10*min((n_countries-2)*0.15, 0.1)
                              )), 3)
...
def compute_pneg(atdi: float, ahifd: float, n_countries: int) -> float:
    ...
    return round(max(0.05, min(0.95,
                               0.70 - atdi/100*0.30 - ahifd/80*0.20
