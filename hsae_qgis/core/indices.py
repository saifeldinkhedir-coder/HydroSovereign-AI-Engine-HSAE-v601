...
def compute_hifd(q_nat: DataPoint, q_obs: DataPoint) -> ProvenancedResult:
    ...
    same_ref = q_nat.source_ref == q_obs.source_ref
    same_src = q_nat.source == q_obs.source
    if same_ref and same_src:
        return insufficient(
            "HIFD",
...
def atdi_for_basin(reg: DataRegistry, basin_id: str) -> ProvenancedResult:
    ...
    inflow = [r.data_point for r in reg.records(basin_id)
              ...
              if r.data_point.is_legal_grade()]
...
def compute_afsf(observed_anomaly: DataPoint,
    ...
    if gate is not None:
        return gate
    same_src = observed_anomaly.source == natural_anomaly.source
    same_ref = observed_anomaly.source_ref == natural_anomaly.source_ref
    if same_src and same_ref:
        return insufficient(
            "AFSF",
    ...
    if rng <= 0:
        return insufficient("AFSF", "signal range must be positive",
                            [observed_anomaly, natural_anomaly, signal_range])
    diff = float(observed_anomaly.value) - float(natural_anomaly.value)
    afsf = abs(diff) / rng
