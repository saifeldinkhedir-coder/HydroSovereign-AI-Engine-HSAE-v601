"""
hsae_qgis/core/indices_scenario.py  —  v6.0.14  (SCENARIO MODE)
================================================================
HEURISTIC SCENARIO formulas for interactive exploration ONLY.

These map user-set parameters (runoff, capacity, dispute level, country
count) to indicative index values for scenario exploration. They are
NOT validated field measurements and must be labelled as such in the UI.

For provenance-bound computation from real observed discharge, use the
clean engine in core/indices.py (provenance + ingestion), which returns
INSUFFICIENT_DATA when observed data are absent rather than a number.
