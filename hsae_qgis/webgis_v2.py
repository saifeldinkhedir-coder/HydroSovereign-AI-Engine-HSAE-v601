...
def build_webgis_v2(basins: list, compute_fn) -> str:
    ...
    for b in basins:
        ...
        hifd = round(d.get('ahifd', d.get('hifd', 0)), 1)  # AHIFD
...
def _compute_atci(atdi: float, hifd: float) -> float:
    """Compute ATCI using calibrated HSAE formula."""
    return round(min(95.0, max(20.0, 100.0 - atdi * 0.5 - hifd * 0.4)), 1)


def _compute_atci_OLD(atdi: float, hifd: float) -> int:
    """Legacy (disabled)."""
    thresholds = [
        ('Art.5', hifd > 25),
        ('Art.7', atdi > 20),
...
<title>HSAE v6.0.12 -- WebGIS Basin Risk Map v2</title>
...
<div id="hdr">
  ...
  <div>
    <div class="htitle">HydroSovereign AI Engine v6.0.12 -- WebGIS Basin Risk Map v2</div>
...
const BASEMAPS = {
  dark: L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png',
    {attribution:'&copy; CARTO &middot; HSAE v6.0.12',subdomains:'abcd',maxZoom:19}),
  light: L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png',
    {attribution:'&copy; CARTO &middot; HSAE v6.0.12',subdomains:'abcd',maxZoom:19}),
  satellite: L.tileLayer(
  'https://server.arcgisonline.com/ArcGIS/rest/services/'
  + 'World_Imagery/MapServer/tile/{z}/{y}/{x}',
    {attribution:'&copy; ESRI &middot; HSAE v6.0.12',maxZoom:19}),
  osm: L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
    {attribution:'&copy; OpenStreetMap &middot; HSAE v6.0.12',maxZoom:19}),
};
...
function colourFor(layer, p) {
  ...
  if (layer === 'ahifd') {
    if (p.ahifd < 15) return '#16a34a';
    if (p.ahifd < 25) return '#ca8a04';
    if (p.ahifd < 35) return '#ea580c';
    return '#dc2626';
  }
  if (layer === 'ci') {
...
function radiusFor(layer, p) {
  const val = layer==='atdi'?p.atdi : layer==='ahifd'?p.ahifd :
...
<div>
  ...
  <div class="p-footer">
    HSAE v6.0.12 &middot; Plugin ID: 5040 &middot; DOI: 10.5281/zenodo.19180160
  </div>
</div>`;
}
...
attrCtrl.onAdd = function() {
  const div = L.DomUtil.create('div', 'legend');
  div.innerHTML = `<b>HSAE v6.0.12</b> &middot; Plugin ID: 5040<br>
