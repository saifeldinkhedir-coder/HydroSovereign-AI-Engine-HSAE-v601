...
def train_ensemble(df: pd.DataFrame, target: str = "Volume_BCM", seed: int = 42) -> dict:
    ...
    rf = RandomForestRegressor(n_estimators=300,max_depth=10,min_samples_leaf=3,n_jobs=-1,random_state=seed)
    rf.fit(Xtr,ytr)

    sc = StandardScaler(); Xts = sc.fit_transform(Xtr)
    mlp = MLPRegressor(hidden_layer_sizes=(128,64,32),activation="relu",solver="adam",max_iter=500,early_stopping=True,
                        validation_fraction=0.1,random_state=seed,learning_rate_init=0.001)
    mlp.fit(Xts,ytr)

    gbm = GradientBoostingRegressor(n_estimators=300,learning_rate=0.05,max_depth=5,subsample=0.8,random_state=seed)
    gbm.fit(Xtr,ytr)
...
def multi_step_forecast(df: pd.DataFrame, horizon: int = 30, target: str = "Inflow_BCM", seed: int = 42) -> dict | None:
    ...
    gbm  = GradientBoostingRegressor(n_estimators=200,learning_rate=0.08,max_depth=4,random_state=seed)
    gbm.fit(X[:sp],y[:sp])
    yp   = gbm.predict(X)
